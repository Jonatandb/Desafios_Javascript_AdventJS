...
rojo y que además su nombre contenga tanto las letras n Y a,
...
function contarOvejas(ovejas) {
  return ovejas.reduce((acc, curr) => {
    if (
      curr.color === 'rojo' &&
      curr.name.toLowerCase().includes('a') &&
      curr.name.toLowerCase().includes('n')
    )
      acc.push(curr)
    return acc
  }, [])
